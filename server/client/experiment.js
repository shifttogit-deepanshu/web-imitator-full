...
let leftArmVector, leftForeArmVector, leftShoulderVector, leftShoulderHelper
...
if(leftArm){	
	...
	leftArmVector = new THREE.Vector3()
	leftArmVector.subVectors(new THREE.Vector3(-landmarks[14].x,-landmarks[14].y,-landmarks[14].z),new THREE.Vector3(-landmarks[12].x,-landmarks[12].y,-landmarks[12].z)).normalize()
	if(leftArmHelper){
		scene.remove(leftArmHelper)
	}
	leftArmHelper = new THREE.ArrowHelper(leftArmVector,new THREE.Vector3(leftArm.getWorldPosition().x,leftArm.getWorldPosition().y,leftArm.getWorldPosition().z),1,"#ffffff")
	scene.add(leftArmHelper)

	leftShoulderVector =  new THREE.Vector3()
	leftShoulderVector.subVectors(new THREE.Vector3(0,0,0),new THREE.Vector3(-1,0,0)).normalize()

	if(leftShoulderHelper){
		scene.remove(leftShoulderHelper)
	}
	leftShoulderHelper = new THREE.ArrowHelper(leftShoulderVector,new THREE.Vector3(leftShoulder.getWorldPosition().x,leftShoulder.getWorldPosition().y,leftShoulder.getWorldPosition().z),1,"#ffffff")
	scene.add(leftShoulderHelper)

	leftForeArmVector = new THREE.Vector3()
	leftForeArmVector.subVectors(new THREE.Vector3(-landmarks[16].x,-landmarks[16].y,-landmarks[16].z),new THREE.Vector3(-landmarks[14].x,-landmarks[14].y,-landmarks[14].z)).normalize()
	if(leftForeArmHelper){
		scene.remove(leftForeArmHelper)
	}
	leftForeArmHelper = new THREE.ArrowHelper(leftForeArmVector,new THREE.Vector3(leftForeArm.getWorldPosition().x,leftForeArm.getWorldPosition().y,leftForeArm.getWorldPosition().z),1,"#ff0000")
	scene.add(leftForeArmHelper)

	var coords = { x:0}; // Start at (0, 0)
	var tween = new TWEEN.Tween(coords) // Create a new tween that modifies 'coords'.
	.to({ x:2800}, 50) // Move to (300, 200) in 1 second.
	.easing(TWEEN.Easing.Quadratic.Out) // Use an easing function to make the animation smooth.
	.onUpdate(function() { // Called after tween.js updates 'coords'.
	// Move 'box' to the position described by 'coords' with a CSS translation.
	
	leftArm.quaternion.rotateTowards(leftShoulderHelper.getWorldQuaternion().invert().multiply(leftArmHelper.getWorldQuaternion()),coords.x*0.0001)
	leftForeArm.quaternion.rotateTowards(leftArmHelper.getWorldQuaternion().invert().multiply(leftForeArmHelper.getWorldQuaternion()),coords.x*0.0001)
	leftShoulder.quaternion.rotateTowards(leftShoulderHelper.getWorldQuaternion(),2)
	...
	// let shoulderHelper = new THREE.ArrowHelper(new THREE.Vector3(1,0,0).normalize(),new THREE.Vector3(1,0,0),1,"#ffffff")
	// leftShoulder.applyQuaternion(shoulderHelper.getWorldQuaternion())
	// leftArm.applyQuaternion(shoulderHelper.getWorldQuaternion().invert())
}
}


function handlePoseLandmarks(landmarks){	
	if(!isPerforming && leftArm){
